...
//	jd["everytime"] = arg.EveryTime
	...
	game := NewGame5GLogic(a,h2["player1"].(int),h2["player2"].(int),h2["time"].(int),h2["everytime"].(int))
	...
	data1.Uid = h2["player1"].(int)
	data1.ConnectId = h2["player1ConnectId"].(int)
	jd := &datamsg.SC_NewGame{}
	jd.GameId = game.GameId
	a.WriteMsgBytes(datamsg.NewMsg1Bytes(data1,jd))
	
	data1.Uid = h2["player2"].(int)
	data1.ConnectId = h2["player2ConnectId"].(int)
	a.WriteMsgBytes(datamsg.NewMsg1Bytes(data1,jd))
}
